...
import { Hyperspeed, PALETTES, type HyperspeedProps } from "./Hyperspeed";

export default function Demo(values: ControlValues) {
  const props = values as unknown as HyperspeedProps;
  const ink = PALETTES[props.palette ?? "blue"]?.ink ?? "#ffffff";
  return (
    <div className="relative h-full w-full overflow-hidden">
      ...
      <div className="pointer-events-none relative z-10 flex h-full flex-col justify-end p-8" style={{ color: ink }}>
        ...
        <h2 className="max-w-md text-3xl font-semibold tracking-tight md:text-4xl">Işık hızında bir tünel.</h2>
      </div>
    </div>
  );
}
